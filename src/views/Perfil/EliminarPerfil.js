...
export default function EliminarPerfil({ navigation }) {
  ...
  const deleteUser = async (contraseña) => {
    ...
    const peticion = await axios.post("http://192.168.0.176:8080/app_movil_sensor/api/user/delete-user", notification, headers)
      .then(async res => {
    ...
    <View style={Styles.container} >
      ...
      <View style={Styles.formContainer}>
        ...
        <View style={Styles.input}>
          ...
          >
            {data.secureTextEntry ?
              <FontAwesomeIcon icon={faEyeSlash} style={[Styles.iconoOjo, { marginLeft: 80 }]} />
              :
              <FontAwesomeIcon icon={faEye} style={[Styles.iconoOjo, { marginLeft: 115 }]} />
            }
          </TouchableOpacity>
        </View>
        {errorConfirm !== null ?
